...
describe('Challenge Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./challenge-detail.component').then(m => m.ChallengeDetailComponent),
              resolve: { challenge: () => of({ id: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load challenge on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', ChallengeDetailComponent);

      // THEN
      expect(instance.challenge()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
