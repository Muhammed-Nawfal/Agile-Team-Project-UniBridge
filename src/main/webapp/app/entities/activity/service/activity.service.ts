...
type RestOf<T extends IActivity | NewActivity> = Omit<T, 'activityDate' | 'createdOn' | 'updatedOn'> & {
...
export class ActivityService {
  ...
  protected convertDateFromClient<T extends IActivity | NewActivity | PartialUpdateActivity>(activity: T): RestOf<T> {
    return {
      ...activity,
      activityDate: activity.activityDate?.toJSON() ?? null,
      createdOn: activity.createdOn?.toJSON() ?? null,
      updatedOn: activity.updatedOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restActivity: RestActivity): IActivity {
    return {
      ...restActivity,
      activityDate: restActivity.activityDate ? dayjs(restActivity.activityDate) : undefined,
      createdOn: restActivity.createdOn ? dayjs(restActivity.createdOn) : undefined,
      updatedOn: restActivity.updatedOn ? dayjs(restActivity.updatedOn) : undefined,
    };
  }
