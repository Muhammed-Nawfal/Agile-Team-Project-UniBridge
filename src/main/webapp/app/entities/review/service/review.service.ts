...
type RestOf<T extends IReview | NewReview> = Omit<T, 'datePublished'> & {
...
export class ReviewService {
  ...
  protected convertDateFromClient<T extends IReview | NewReview | PartialUpdateReview>(review: T): RestOf<T> {
    return {
      ...review,
      datePublished: review.datePublished?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReview: RestReview): IReview {
    return {
      ...restReview,
      datePublished: restReview.datePublished ? dayjs(restReview.datePublished) : undefined,
    };
  }
