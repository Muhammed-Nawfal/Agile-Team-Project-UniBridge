...
/**
 * REST controller for managing {@link bham.team.domain.Ranking}.
 */
@RestController
...
public class RankingResource {
    ...
    public ResponseEntity<Ranking> createRanking(@Valid @RequestBody Ranking ranking) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/rankings/" + ranking.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, ranking.getId().toString()))
            .body(ranking);
    }
    ...
    public ResponseEntity<Ranking> partialUpdateRanking(
        ...
        Optional<Ranking> result = rankingRepository
            ...
            .map(existingRanking -> {
                if (ranking.getReviewNumber() != null) {
                    existingRanking.setReviewNumber(ranking.getReviewNumber());
                }
                if (ranking.getActivityNumber() != null) {
                    existingRanking.setActivityNumber(ranking.getActivityNumber());
                }
                if (ranking.getStarAverage() != null) {
                    existingRanking.setStarAverage(ranking.getStarAverage());
                }
                if (ranking.getReliable() != null) {
                    existingRanking.setReliable(ranking.getReliable());
                }
